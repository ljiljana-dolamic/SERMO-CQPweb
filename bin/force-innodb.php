...
$result = do_mysql_query("select table_name from information_schema.tables where table_schema = '{$Config->mysql_schema}'  and engine  = 'InnoDB'");
echo number_format(mysql_num_rows($result)), " in your database already use the InnoDB engine.\n";
$result = do_mysql_query("select table_name from information_schema.tables where table_schema = '{$Config->mysql_schema}'  and engine <> 'InnoDB'");
echo number_format(mysql_num_rows($result)), " in your database use the older MyISAM engine.\n";
...
/* the pointy end! */
$result = do_mysql_query("select table_name from information_schema.tables where table_schema = '{$Config->mysql_schema}'");
while (false !== ($r = mysql_fetch_row($result)))
{
	if (! in_array($r[0], $dont_innodbise))
	{
		do_mysql_query("alter table `{$r[0]}` ENGINE=InnoDB");
		$upgraded_to_idb++;
