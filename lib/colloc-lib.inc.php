...
function load_statistic_info()
{
	...
	$info[0]['desc'] = 'Par ordre de fréquence';
...
function run_script_for_solo_collocation()
{
	...
	if (QueryScope::TYPE_WHOLE_CORPUS == $query_record->qscope->type)
		$basis_point = 'le corpus entier';
		else
			$basis_point = 'le sous-corpus';
			
			echo print_html_header($Corpus->title . ' -- résultats de collocation', $Config->css_path);
	...
	echo "L'information sur la collocation de pivot  &ldquo;"
 		, escape_html($query_record->cqp_query)
		, "&rdquo; avec &ldquo;$soloform_html&rdquo; $tag_description $bdo_tag1($basis_to_show occurrences dans $basis_point)$bdo_tag2 "
 		;
	...
	echo "
		...
		<tr>
			<th class=\"concordtable\" width=\"50%\">Type de statistic</th>
			<th class=\"concordtable\" width=\"50%\">
				Valeur (pour empan de $calc_range_begin à $calc_range_end)
			</th>
		</tr>"
 		;
	...
	echo "
		<tr>
			<th class=\"concordtable\" colspan=\"4\">
				Dans l'empan de $calc_range_begin à $calc_range_end, <em>$soloform_html</em> apparaît 
				$observed_to_show fois dans 
				$number_of_texts_to_show textes distincts
				(fréquence attendu: $expected_to_show)
			</th>
		</tr>
		<tr>
			<th class=\"concordtable\" align=\"left\">Distance</th>
			<th class=\"concordtable\">N° de occurrences</th>
			<th class=\"concordtable\">Dans n° de textes</th>
	...
	for ($i = $calc_range_begin ; $i <= $calc_range_end ; $i++)
	{
		...
		else
		{
			$link = "<a href=\"concordance.php?qname={$query_record->qname}&newPostP=coll"
				...
				. "&uT=y\" onmouseover=\"return escape('Afficher les résultats de collocation avec "
				. "<B>" . addslashes($soloform_html) . "</B> en position <B>$i</B>')\">$i</a>";
	...
	)
{
	...
	echo "N°\t$att_desc\tN° total dans $basis_desc\tFréquence attendue\t"
		. "Fréquence observée\tDans n° de sermons $sighead";
	echo "$eol$eol";
