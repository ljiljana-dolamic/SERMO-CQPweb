...
function printquery_history()
{
	...
	else
	{
		...
		<table class="concordtable" width="100%">
			<tr>
				<th colspan="<?php echo $column_count; ?>" class="concordtable">Mes requêtes</th>
			</tr>
		<?php
	}
	...
	?>
		<tr>
			...
			<?php if ($usercolumn) echo '<th class="concordtable">Utilisateur</th>'; ?>
			<th class="concordtable">Requête </th>
			<th class="concordtable">Restriction</th>
			<th class="concordtable">N° résultats</th>
			<th class="concordtable">Date</th>
			<?php if ($delete_lines) echo '<th class="concordtable">Effacer</th>'; ?>
	...
	for ( $i = 1 ; $i < $toplimit ; $i++ )
	{
		...
		if ( $view == 'simple' && $row['simple_query'] != "" )
			echo '<td class="concordgeneral"><a class="queryCQP" href="index.php?thisQ=search&insertString=' 
				, urlencode($row['simple_query']) , '&insertType=' , $row['query_mode'] , '&uT=y"'
				, ' onmouseover="return escape(\'Insérer la requête dans la fenêtre de requête\')">' 
		...
		if ($qs->type == QueryScope::TYPE_SUBCORPUS)
		{
			...
			if (QueryScope::$DELETED_SUBCORPUS == $qs->serialise())
				echo '<td class="concordgeneral">(sous-corpus effacé)</td>';
			else
				echo '<td class="concordgeneral">Sous-corpus:<br/><a class="queryCQP" href="index.php?thisQ=search&insertString='
					...
					, ' onmouseover="return escape(\'Insérer la requête et le sous-corpus dans la fenêtre de requête\')">'
					, $subc_mapper[(int)$qs->serialise()]
					, '</a></td>'
					;
		}
		else if ($qs->type == QueryScope::TYPE_RESTRICTION)
			echo '<td class="concordgeneral"><a class="queryCQP" href="index.php?thisQ=restrict&insertString='
				, urlencode(($view == 'simple' && $row['simple_query'] != "") ? $row['simple_query'] : $row['cqp_query']) 
				, '&insertType=' , ( $view == 'simple' ? $row['query_mode'] : 'cqp' ) 
		...
		case -3:
			echo "<td class='concordgeneral' align='center'>Erreur d'exécution</a></td>";
			break;
		case -1:
			echo "<td class='concordgeneral' align='center'>Erreur de syntaxe</td>";
			break;
		default:
			if ($qs->type == QueryScope::TYPE_SUBCORPUS)	
			{
				if (QueryScope::$DELETED_SUBCORPUS == $qs->serialise())
					...
					echo "<td class='concordgeneral' align='center'><a href=\"concordance.php?"
						...
						, "&qmode=cqp&uT=y\" onmouseover=\"return escape('Recréer le résultat de la requête')\">" 
						, $row['hits'] , "</a></td>"
						;
			}
			else if ($qs->type == QueryScope::TYPE_RESTRICTION)
	...
	$navlinks .= '">&lt;&lt; [Requêtes plus récentes]';
	if ($begin_at > 1)
		$navlinks .= '</a>';
	...
	$navlinks .= '">[Requêtes plus anciennes] &gt;&gt;';
	if (mysql_num_rows($result) > $i)
		$navlinks .= '</a>';
		...
		<tr>
			<td class="concorderror" colspan="3">
				...
				if (!$Config->hide_experimental_features)
				{
					...
					?>
					&nbsp;<br>
					<center><a href="index.php?thisQ=clocurve&uT=y">Click here for the experimental Closure Curve tool</a>.</center>
					<br>&nbsp;
					<?php 
...
function printquery_clocurve()
{
	global $Corpus;
	global $Config;

	if ($Config->hide_experimental_features)
		exiterror("clocurves are an experimental feature, disabled in your system.");
	
	// TODO 
	?>
	<table class="concordtable" width="100%">
		<tr>
			<th class="concordtable" colspan="8">Closure Curve Tool</th>
		</tr>
		<tr>
			<td class="concorderror" colspan="8">
				&nbsp;<br>
				<b>WARNING</b> - Closure Curves are currently <u>experimental</u>.
				<br>&nbsp;
			</td>
		</tr>
		<tr>
			<th class="concordtable" colspan="8">Closure curves currently stored on the system</th>
		</tr>
		<tr>
		...
		<?php
		//TODO uncommment!!!
		$cc_list = list_corpus_clocurves($Corpus->name);
//$cc_list = array();
		
		if (empty($cc_list))
			echo "\n\t\t<tr><td class=\"concordgrey\" colspan=\"8\">Currently no Closure Curve data exists for this corpus.</td></tr>";
		
		foreach ($cc_list as $clocurve)
			echo "\n\t\t<tr>"
 				, "\n\t\t\t<td class=\"concordgrey\">" , $clocurve->id , "</td>"
 				, "\n\t\t\t<td class=\"concordgeneral\">" , $clocurve->annotation , "</td>"
 				, "\n\t\t\t<td class=\"concordgeneral\">" , number_format($clocurve->interval_width), "</td>"
 				, "\n\t\t\t<td class=\"concordgeneral\">" , $clocurve->n_datapoints , "</td>"
 				, "\n\t\t\t<td class=\"concordgeneral\">" , date(CQPWEB_UI_DATE_FORMAT, $clocurve->create_time), "</td>"
 				, "\n\t\t\t<td class=\"concordgeneral\">" , number_format($clocurve->create_duration), " s</td>"
 				, "\n\t\t\t<td class=\"concordgeneral\" align=\"center\">" 
 					, '<a class="menuItem" href="clocurve-admin.php?ccAction=download&clocurve=', $clocurve->id, '&uT=y">[Download]</a></td>'
 				, "\n\t\t\t<td class=\"concordgeneral\" align=\"center\">" 
 					// TODO this DEFINTEILY needs "are you sure? modal dialog protection jQuery stuff./
 					, '<a class="menuItem" href="clocurve-admin.php?ccAction=delete&ccToDelete=', $clocurve->id, '&uT=y">[Delete]</a></td>'
 				, "\n\t\t</tr>"
 				;
	...
	<table class="concordtable" width="100%">
		...
		?>
		<form action="clocurve-admin.php" method="get">
			<tr>
			...
			<input type="hidden" name="ccAction" value="generate" />
			<input type="hidden" name="uT" value="y" />
		</form>
	</table>
