...
// 	private $subcorpus;// retained for the nonce as a private, so that if anything references it directly, things will break.
	 ...
	 * 
	 * The query_scope field follows the format outlined in subcorpus.inc.php. It uses "" as the "empty" value, 
...
// 	private $scope_reduction_whereclause = NULL;
	...
	{
		$result = do_mysql_query("SELECT * from saved_queries where query_name = '". mysql_real_escape_string($qname) . "'");
	...
	public function load_from_params($corpus, $cqp_query, $query_scope = '', $postprocess = '')
	{
		...
		if (is_object($query_scope))
		{
			...
			$sql_query_scope = ( QueryScope::TYPE_WHOLE_CORPUS == $this->qscope->type ? '' : mysql_real_escape_string($this->qscope->serialise()) );
		}
		else
			$sql_query_scope  = (empty($query_scope) ? '' : mysql_real_escape_string($query_scope));
		
		$postprocess  = mysql_real_escape_string((string)$postprocess);
	 ...
	 *  
	 * @param resource $result  MySQL result containing the query record from ``saved_queries''.
	 ...
	 *                          in which case false).
	 */
	public function load_from_db_result($result, $check = false)
	{
		if ($check)
		{
	...
	private function load($result)
	{
		...
		else
		{
			...
			do_mysql_query( "DELETE FROM saved_queries where query_name = '{$this->qname}'");
			return false;
		}
	}
	...
	public function save()
	{
		...
		$simple_query = mysql_real_escape_string($this->simple_query);
		$cqp_query    = mysql_real_escape_string($this->cqp_query);
		$query_scope  = mysql_real_escape_string($this->query_scope);
		$postprocess  = mysql_real_escape_string($this->postprocess);
	...
	public function do_postprocess($descriptor)
	{
		...
		$this->qname = $new_qname = qname_unique($Config->instance_name);
...
// 		 		= mysql_fetch_row(do_mysql_query(
...
// 					; 
// 		return mysql_fetch_row(do_mysql_query("select sum(words), count(*) from text_metadata_for_{$this->corpus} $where"));
	}
...
// 		$this->postprocess_stack  = ('' === $this->postprocess ? array() : explode('~~', $this->postprocess));
	...
	public function touch()
	{
		...
		do_mysql_query("update saved_queries set time_of_query = {$this->time_of_query} where query_name = '{$this->qname}'");
		
		/* following is slightly suspect, but hey */
		global $mysql_link;
		return (1 == mysql_affected_rows($mysql_link));
	}
	 ...
	 * information about the size of the corpus is optional, pass false as 1st argument to turn off.
	 * 
	 * To get plaintext instead of HTML, pass false as 2nd argument. 
	 */
	public function print_solution_heading($include_corpus_size = true, $html = true)
	{
		...
		if ($this->query_mode == 'uploaded')
			...
			{
				$final_string = 'Requête: ' . ($html?'&ldquo;':'"');
			...
			if (! empty($desc))
				...
				$final_string .= "<br/>Fréquence du pivot: " . number_format((float)$this->hits,0,"."," ") . ' ';
				
				
				if ($this->hit_texts > 1)
					$final_string .= ' dans ' . number_format((float)$this->hit_texts) . ' textes';
					else
						$final_string .= ' dans 1 texte';
						
						$include_corpus_size= false; /*forcing to fales*/
	 ...
	 * 
	 * (This may not, strictly speaking, need to be a separate method to print_solution_heading...)
	 * 
	...
	public function print_postprocess_description($html = true)
	{
		...
		$r_2_l = (bool) get_corpus_metadata('main_script_is_r2l', $this->corpus);
		...
		$annotations = get_corpus_annotations($this->corpus);
...
function delete_stray_cache_entry($qname)
{
	...
	foreach ($qname as $q)
		do_mysql_query("DELETE from saved_queries where query_name = '" . mysql_real_escape_string($q) . '\'');
}
...
function delete_cached_query($qname)
{
	...
	do_mysql_query('DELETE from saved_queries where query_name = \'' . mysql_real_escape_string($qname) . '\'');
...
function copy_cached_query($oldqname, $newqname)
{
	...
	$q = QueryRecord::new_from_qname($oldqname);
	if (false === $q)
		return false;
...
function touch_cached_query($qname)
{
	...
	do_mysql_query("update saved_queries set time_of_query = $time_now where query_name = '$qname'");
}
...
function catquery_get_categorisation_table($qname, $from, $to)
{
	...
	$result = do_mysql_query("select refnumber, category from $dbname where refnumber >= $from and refnumber <= $to");
...
function catquery_find_dbname($qname)
{
	...
	if (mysql_num_rows($result) < 1)
		exiterror_general("The categorised query ``$qname'' could not be found in the database.");
...
function history_delete($instance_name)
{
	...
	do_mysql_query("delete from query_history where instance_name = '$instance_name'");
}
