...
case 'userLogin':
	...
	if ( false === ($userinfo = check_user_password($username_for_login, $_POST['password'])))
	{
		...
		exiterror_login(array(
			"Cette combinaison nom d'utilisateur / mot de passe n'a pas été reconnue.",
			"Vous avez peut-être mal saisi le nom d'utilisateur ou le mot de passe (les deux sont sensibles à la casse).",
	...
	else
	{
		...
		case USER_STATUS_UNVERIFIED:
			exiterror_login(array(
				"Vous ne pouvez pas vous connecter à ce compte car il n'a pas encore été activé.",
				"Veuillez utiliser le lien dans le message de vérification envoyé à votre adresse e-mail pour activer ce compte."
				));
...
case 'newUser':
	...
	if ($script_called_from_admin)
		$next_location = "index.php?thisF=showMessage&message=" . urlencode("Le compte '$new_username'  a été créé.") . "&uT=y";
	else
		$next_location = "index.php?extraMsg=" . urlencode("Le compte '$new_username' a été créé.") . "&uT=y";
...
case 'remindUsername':
	...
	if (!isset($_GET['emailToRemind']))
		exiterror("Aucune adresse email spécifiée! Veuillez revenir en arrière et réessayer.");
	...
	if (1 > mysql_num_rows($result))
		exiterror(array(
			"Aucun compte avec l'adresse e-mail suivante n'a été trouvé sur le système:",
			$_GET['emailToRemind'], 
...
Un rappel d'identifiant a été demandé pour cette adresse e-mail pour SERMO-CQPweb.

Le nom d'utilisateur associé à votre e-mail est le suivant:
...
HERE;
		...
		send_cqpweb_email($user_address, "SERMO: rappel de nom d'utilisateur", $body);
	}
	
	/* but the message assumes just one email, since that's the normal case */
	$next_location = "index.php?thisQ=welcome&extraMsg=" . urlencode("Un e-mail de rappel avec votre nom d'utilisateur a été envoyé.") . "&uT=y";
...
case 'requestPasswordReset':
	...
	if (!isset($_GET['userForPasswordReset']))
		exiterror("Aucun nom d'utilisateur fourni. Veuillez revenir en arrière et réessayer.");

	if ( ! in_array($_GET['userForPasswordReset'], get_list_of_users()) )
		exiterror("Nom d'utilisateur invalide! Veuillez revenir en arrière et réessayer.");
...
Une réinitialisation de mot de passe a été demandée pour votre compte d'utilisateur sur SERMO-CQPweb.

Si vous voulez vraiment réinitialiser votre mot de passe, vous pouvez utiliser ce code  sur le formulaire de réinitialisation du mot de passe:

    $vcode_render

Le formulaire peut être accédé à l'URL suivante:

    $abs_url

Si vous ne voulez pas réinitialiser votre mot de passe, ignorez cet e-mail.
