...
class QueryScope
{
	 ...
	 * @return QueryScope         The new object.
	 */
	public static function new_from_post($t_param)
	{
		$o = new self();
		$o->parse_from_post($t_param);
		return $o;
	}
...
//			$sql = "SELECT restrictions from saved_subcorpora
	...
	/**
	 * This func  extracts a subcorpus from the psot parameter in the assisted query.
	 *
	 * The  arg the value of the t parameter in the post
	 */
	private function parse_from_post($t_param)
	{
		/* initial conditions: whole corpus. May be disproven below. */
		$this->serialised = '';
		$this->type = self::TYPE_WHOLE_CORPUS;
		
		if (preg_match('/~sc~(\d+)/', $t_param, $m))
		{
			/* query is within a subcorpus. */
			$this->subcorpus = Subcorpus::new_from_id($m[1]);
			$this->serialised = $m[1];
	...
	public function print_description($html = true)
	{
		...
		case self::TYPE_SUBCORPUS:
			return 'dans sous-corpus' . ($html?'&ldquo;<em>':'"') . $this->subcorpus->name . ($html?'</em>&rdquo;':'"');
		}
	}
