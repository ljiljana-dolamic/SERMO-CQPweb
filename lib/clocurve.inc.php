...
/**
 * @file This file contains functions for creating and manipulating closure curves.
 */
 ...
 * 
 * "clocurve" is the abbreviation used in-code for "corpus closure curve". 
 * 
 * We use two tables to represnt clocurves: one for the info, and all datapoints go in a single table. 
 * 
 * 
...
TWO ALTERNATIVES FOR THE ALGORITHM
		...
		when done,
			for each $new_types_per_interval
				write one entry to the clocurve_data table.
...
=================================================================================================================
		...
		for i = 0, n =count(new_types_in_interval) ;  i < n ; i ++
		{
			...
			if coujnt($tuples) > 99
				 nsert into clocurve_datapoints (closurve_id, tokens, cum_type) VALUES implode(tuples)
				 tuples = array
		}
		
		if (not em pty tyupers)
		
			insert into clocurve_datapoints (closurve_id, tokens, cum_type) VALUES implode(tuples)
...
/**
 * Checks for the existence of a clocurve, given the same three defining
 * parameter used to request generation of said clocurve: the corpus, the annotation, the interval.
 * 
 * @see generate_clocurve_data
 * @param  string $corpus          Corpus handle: as in generate_clocurve_data
 * @param  string $annotation      Annotation handle: as in generate_clocurve_data
 * @param  int    $interval_width  Token interval: as in generate_clocurve_data 
 * @return int                     Returns the integer ID of the clocurve, if it exists. If not, returns boolean false. 
 */
function check_clocurve_parameters($corpus, $annotation, $interval_width)
{
	$corpus = mysql_real_escape_string($corpus);
	$annotation = mysql_real_escape_string($annotation);
	$interval_width = (int) $interval_width;
	
	$result = do_mysql_query("select id from clocurve_info where corpus = '$corpus' and annotation = '$annotation' and interval_width = $interval_width");
...
/**
 * Gets the information on aspecifired clocurve from the database.
 * 
 * @param  int      $id  ID of the clocurve whose information is required.
 * @return stdClass      Database object containing the attrributes of this Clocurve datraset; false if id not foudn.
 */
function get_clocurve_info($id)
{
	$id = (int) $id;
	
	$result = do_mysql_query("select * from clocurve_info where id = $id");
...
/**
 * Get database objects for all the clocurves that currently exist in the specified corpus.
 * 
 * @param  string $corpus  Corpus to look for clocurves in.
 * @return array           Array of database objects. May be empty if none exist. Keys are the ID numbers.
 */
function list_corpus_clocurves($corpus)
{
	$corpus = cqpweb_handle_enforce($corpus);
	
	$result = do_mysql_query("select * from clocurve_info where corpus = '$corpus' order by id asc");
...
/**
 * Delete a clocurve (record plus all datapoints) based on its integer reference.
 * 
 * @param  int $id  Integer ID of the clocurve to delete.
 */
function delete_clocurve($id)
{
	global $User;
...
// 	Later I may want to make it possible for closure curve data that is old to delete itself to save space,
// 	for now tho deletion is an admin action.
	
	$id = (int) $id;
	
	/* this is a v  straightforward procedure! */
	do_mysql_query("delete from clocurve_datapoints where clocurve_id = $id");
	do_mysql_query("delete from clocurve_info where id = $id");
}


/**
 * Creates a clocurve with the specified feature triplet.
 * 
 * DELEGATOR FUNCTION FOR THE TWO ALGORITHMS BEING EXPRIMENTED ON.
 * 
 * Will take no action if the specified closure curve already exists. 
 * 
 ...
 */
function create_clocurve($corpus, $annotation, $interval_width)
{
//	return create_clocurve_bytypes($corpus, $annotation, $interval_width);
	return create_clocurve_bruteforce($corpus, $annotation, $interval_width);
}
 ...
 * @return bool                      True for success: false if this curve already existed.
 */
function create_clocurve_bytypes($corpus, $annotation, $interval_width)
{
	global $cqp;
	
	$check_id = check_clocurve_parameters($corpus, $annotation, $interval_width);
	
	if (is_int($check_id))
		return false;
	/* a clocurve for that data with that interval already exists. So we do nothing. */
	
	/* just a wee sanity check... even 100 is on the very small side for most corpora. */
	if ($interval_width < 100)
		exiterror("Cannot create a clocurve with an interval of less than 100 tokens");
	
	/* also a sanity check, caller ought to make sure $corpsu is real  */
	if (false === ($c_info = get_corpus_info($corpus)))
		exiterror("Cannot create a clocurve for tghe given corpus handle becausew it does not exist.");	
	
	
	/* create a record for this clocurve, and get its ID. */ 
	$corpus = mysql_real_escape_string($corpus);
	$annotation = mysql_real_escape_string($annotation);
	$interval_width = (int) $interval_width;
	
	do_mysql_query("insert into clocurve_info (corpus, annotation, interval_width) values ('$corpus', '$annotation', $interval_width)");
	
	$clocurve_id = get_mysql_insert_id();
...
// 	for ($i = 0; $c_info->size_tokens >= $interval_width*($i+1) ; $i++) // off by one bug
	...
	while (1)
	{
		...
		if (is_null($result) || false === ($r = mysql_fetch_row($result)))
		{
			$result = do_mysql_query("select item from freq_corpus_{$corpus}_{$annotation} LIMIT $next_limit_begin, 10000");
...
// commented out debug - to delete later
// $ret = $cqp->execute('dump ClosQ 0 0');
// show_var($ret);
// if (false === $ret) 
// 	exiterror($cqp->get_error_message_as_html());
// list($rawdump) = $ret;
		/* the first number on the line is the cpos of the first instance of the type. */
	...
	for ($i = 0 ; $i < $n_datapoints ; $i++)
	{
		...
		/* add an SQL tuple of the foreign key to the clocurve ID, then the tokens, then the cumulative types. */
		$tuples[] = "($clocurve_id,$tokens_after_interval,$cumulative_n_types)";
		/* e.g. id, 1000, 12      id, 2000, 24,     id, 3000, 36      .....     id, 9980, 100 */ 

		if (499 < count($tuples))
		{
			do_mysql_query("insert into clocurve_datapoints (clocurve_id,tokens,types_so_far) VALUES " . implode(',', $tuples)); 
			$tuples = array();
		}
	}
	...
	/* finally: update the clocurve_info entry to contain 
	...
	do_mysql_query("
			update clocurve_info set
				create_time     = $create_time,
				create_duration = $create_duration,
				n_datapoints    = $n_datapoints
			where id = $clocurve_id");
 ...
 * @return bool                      True for success: false if this curve already existed.
 */
function create_clocurve_bruteforce($corpus, $annotation, $interval_width)
{
	global $Config;
	...
	if (false === ($c_info = get_corpus_info($corpus)))
		exiterror("Cannot create a clocurve for the given corpus handle becausew it does not exist.");	
	...
	/* in order to know how to casefold, we need to know what the underlying charset is */
	if ( 'word' == $annotation && ! $c_info->uses_case_sensitivity )
	{
		...
		case 'utf8':
			if (function_exists('mb_strtolower'))
				...
				exiterror("Cannot build a closure curve for a case-insensitve corpus without PHP's mb_strtolower() function installed! ");
			break;
		default:
	...
	if (false === ($source = popen($cwb_cmd, "r")))
		exiterror("Couldn't open cwb-decode to read p-attribute for closure curve!!");
	...
	while (false !== ($token = fgets($source)))
	{
		...
		{
			$tuples[]= "($clocurve_id,$total_tokens_seen,$cumulative_n_types)";
			++$n_datapoints;
...
// debug code.
print_debug_message("Max memory used for building closure curve: " . memory_get_peak_usage());
...
/**
 * Sends the content of a closure curve to the browser as a text-file download.
 * 
 ...
 * 
 * @param int  $id           ID of the clocurve to Download.
 * @param bool $header_line  If true, file has a header line. If false, just the graph coordinates.
 */
function download_clocurve($id, $header_line = true)
{
	global $User;
	...
	if (false === ($cc = get_clocurve_info($id)))
		exiterror("The closure curve that you have tried to download does not seem to exist. It may have been deleted.");
	
	/* assemble the filename from info in the clocurve obj */
	$filename = 'clocurve-' . $cc->corpus . '-' . $cc->annotation . '.txt';
	...
	/* get the closure curve and write to output attachment! */
	$result = do_mysql_query("select tokens as X, types_so_far as Y from clocurve_datapoints where clocurve_id = $id order by X asc");
...
// and then some funcs will be needed to do stuff with R graphs.... no time for that right now, just use Excel!

// load ot R? load more than 1 to R, so we can overlay curves for comparison

// later funcs like "check N opf closure curves, and hoiw mcuh disk spacer they need" will be wanted...

// note, this does not cover subcorpoira. the algo would have to be totally different I reckon...





// a function to scrub all clocurves if the case-sensitivity of the corpuis changes????????
