...
function printquery_subcorpus()
{
	...
	case 'define_subcorpus':
		...
		print_sc_showsubcorpora();  /* adding info on existing corpora 
		break;
...
function print_sc_nameform($badname_entered, $colspan, $form_method = 'get')
{
	...
	<form action="subcorpus-admin.php" method="<?php echo $form_method; ?>">
	 <table class="concordtable" width="100%">
<!-- 		<tr> -->
			<!--  th class="concordtable"--><!--?php echo $colspan_text; ?--><!--Créer nouveau sous-corpus</th-->
<!-- 		</tr> -->
		<?php
		...
		if($badname_entered)
		{
			...
			<tr>
				<td class="concorderror" align="center" <?php echo $colspan_text; ?>>
					...
					Le nom que vous avez saisi, &ldquo;<?php echo escape_html($_GET['subcorpusNewName']);?>&rdquo;,
					n'est pas autorisée comme nom de sous-corpus.
				</td>
			</tr>
			<?php
		}
		...
		<tr>
			<td class="concordgeneral"<?php echo $colspan_text; ?>>
				<table align="center">
					<tr>
	 					<td class="basicbox">
							<strong>Saisir le nom du sous-corpus</strong>
							( chiffres, caractères sans accent, tiret bas (&nbsp;_&nbsp;))
						</td>
						<td class="basicbox">
...
function print_sc_define_metadata()
{
	...
	echo printquery_build_restriction_block($insert_r, 'sous-corpus');
...
<!-- 				&nbsp; -->
<!-- 				<br/> -->
<!-- 				Cochez les catégories que vous  souhaitez inclure.  -->
<!-- 				<br/>&nbsp;<br/> -->
...
<!-- 				<br/> -->
				...
				<input name="action" type="submit" value="Créer un sous-corpus"/>
<!-- 				<br/>&nbsp;<br/>&nbsp;<br/> -->
...
function print_sc_showsubcorpora()
{
	...
	<table class="concordtable" width="100%">
		<tr>
			<th class="concordtable" colspan="7">Sous-corpus existants</th>
		</tr>
		<tr>
			<th class="concordtable">Nom</th>
			<th class="concordtable">Composition</th>
			<th class="concordtable">Nr de tokens</th>
			<th class="concordtable">Liste de fréquences</th>
			<!--th class="concordtable" colspan="2">Actions</th-->
			<th class="concordtable">Supprimer</th>
		</tr>
		<?php
...
// 					;
			...
			else
			{
				if ($sc->size_tokens() >= $User->max_freqlist())
					echo '<a class="menuItem" " onmouseover="return escape(\' Ne peut pas compiler les tableau pour ce sous-corpus,'
						, ' il est trop grand (votre limite est: <b>'
						, number_format($User->max_freqlist())
						, '</b> tokens)\')"> Ne peut pas compiler</a>'
						;
				else
					echo '<a class="menuItem" href="subcorpus-admin.php?scriptMode=compile_freqtable&compileSubcorpus=' 
						, $sc->id
						, '&uT=y'
						, '" onmouseover="return escape(\'Calculer les tables de fréquences pour le sous-corpus <b>'
						, $sc->name
						, '</b>, permettant le calcul de collocations\')">Compiler</a>'
						;
			}
			echo '</center></td>';
...
// 				, $sc->id
// 				, '&uT=y" onmouseover="return escape(\'Copier ce suos-corpus\')">'   
// 				, '[copy]</a></td>'
// 				;
...
// 				, $sc->id
// 				, '&uT=y" onmouseover="return escape(\'Ajouter les textes dans ce sous-corpus\')">'   
// 				, '[add]</a></td>'
// 				;
			...
			echo '<td class="concordgeneral" align="center"><a class="menuItem" ' 
				...
				, $sc->id 
				, '&uT=y" onmouseover="return escape(\'Supprimer ce sous-corpus\')">'
				, '[x]</a></td>'
				;
		...
		if (0 == mysql_num_rows($result))
			echo '<tr><td class="concordgrey" colspan="7" align="center">&nbsp;<br/>Aucun sous-corpus.<br/>&nbsp;</td></tr>';	
