...
// 		case 'lemme':
// 		case 'bothLemme':
// 			$secondary_annotation = get_corpus_metadata('secondary_annotation');
// 			$breakdown_of = $_GET['concBreakdownOf'];
// 			if (empty($secondary_annotation))
// 				exiterror_general('You cannot do a frequency breakdown based on annotation, '
// 						. 'because no primary annotation is specified for this corpus.');
// 				break;
		case 'p_att':
	...
	if (isset($_GET['pageNo']))
		$_GET['pageNo'] = $page_no = prepare_page_no($_GET['pageNo']);
			...
			if (isset($_GET['pp']))
				$per_page = prepare_per_page($_GET['pp']);   /* filters out any invalid options */
					...
					$breakdown_heading
					= number_format((float)$db_types_total) . "  type$sing_type distinct$sing_type pour "
					. number_format((float)$db_tokens_total) . " tokens."
							...
							else
							{
								...
								$return_option = ($sql_position=='node'
										? '<option value="concBreakdownNodeSort">Afficher les résultats triés par pivot</option>'
								...
								switch($breakdown_of)
								{
									case 'words':
										...
										=  '<option value="concBreakdownWords" selected="selected">Forme</option>
					<option value="concBreakdownAnnot">Étiquette morpho-syntaxique</option>
					<option value="concBreakdownBoth" >Forme et étiquette morpho-syntaxique</option>';
										break;
									case 'annot':
										$breakdown_of_options
										=  '<option value="concBreakdownWords" >Forme</option>
					<option value="concBreakdownAnnot" selected="selected">Étiquette morpho-syntaxique</option>
					<option value="concBreakdownBoth" >Forme et étiquette morpho-syntaxique</option>';
										break;
									case 'both':
										$breakdown_of_options
										=  '<option value="concBreakdownWords" >Forme</option>
					<option value="concBreakdownAnnot">Étiquette morpho-syntaxique</option>
					<option value="concBreakdownBoth" selected="selected">Forme et étiquette morpho-syntaxique</option>';
										break;
								}
		...
		<form action="redirect.php" method="get">
			...
			<td class="concordgrey">
				<select name="redirect">
					<option value="concBreakdownDownload">Télécharger ( ' . $breakdown_of_info[$breakdown_of]['desc'] . ')</option>'.
					//' . $return_option . '
		...
		';
				...
				echo print_html_header($Corpus->title . ' -- Fréquence', $Config->css_path);
...
<div class="container-wrapper">
		...
		<table class="concordtable" width="100%">
			<tr>
				<th class="concordtable" align="left">N°</th>
				<th class="concordtable" align="left">Resultat</th>
				<th class="concordtable">N° de occurrences</th>
				<th class="concordtable">Pourcent</th>
			</tr>
...
function freqbreakdown_write_download($result, $description, $total_for_percent)
{
	...
	echo "N°\tresultat\tNombre d'occurrences\tPourcent";
	echo "$eol$eol";
