					...
					if (isset($_GET['flFilterString']) && $_GET['flFilterString'] !== '')
					{
						switch($_GET['flFilterType'])
						{
							case 'begin':
								...
								$filter_desc = ", commençant par &ldquo;{$_GET['flFilterString']}&rdquo;";
								break;
							case 'end':
								$filter_clause = "item like '%" . mysql_real_escape_string($_GET['flFilterString']) . "'";
								$filter_desc = ", se terminant par &ldquo;{$_GET['flFilterString']}&rdquo;";
								break;
							case 'contain':
								$filter_clause = "item like '%" . mysql_real_escape_string($_GET['flFilterString']) . "%'";
								$filter_desc = ", contenant &ldquo;{$_GET['flFilterString']}&rdquo;";
								break;
							case 'exact':
								$filter_clause = "item = '" . mysql_real_escape_string($_GET['flFilterString']) . "'";
								$filter_desc = ", corespondant exactement  &ldquo;{$_GET['flFilterString']}&rdquo;";
								break;
									...
									if (isset($_GET['pp']))
										$per_page = prepare_per_page($_GET['pp']);   /* filters out any invalid options */
											...
											if ( !isset($_GET['flTable']) || $_GET['flTable'] == '__entire_corpus' )
											{
												...
												$table_desc = "&ldquo;" . escape_html($Corpus->title) . "&rdquo;";
											}
											else
											{
												...
												if (false === ($subcorpus = Subcorpus::new_from_id($_GET['flTable'])))
													exiterror("Le sous-corpus introuvable.");
													...
													$grand_where = '';
														...
														$grand_where = "where $range_clause";
															...
															$grand_where = "where $filter_clause";
																...
																$count_sql = "SELECT count(item) from $freqtable
																$grand_where
																$order_by_clause";
																
																/* and run it */
																$count_result = do_mysql_query($count_sql);
																list($resultCount) = mysql_fetch_row($count_result);  /*get total number of results*/
																
																$pageCount = (int)($resultCount/$per_page) + 1;
																...
																$description =
																($download_mode
																		? "La liste de fréquences: Les fréquences de {$att_desc[$att]} dans {$table_desc}{$filter_desc}{$range_desc}"
																		...
																		: "La liste de fréquences: Les fréquences de <em>".escape_html($att_desc[$att])."</em> dans {$table_desc}{$filter_desc}{$range_desc}"
																		);
																...
																if ($download_mode)
																{
																	freqlist_write_download($att_desc[$att], $description, $result, $sum_tokens, $n);
																...
																else
																{
																	...
																	echo print_html_header($Corpus->title . ' -- la liste de frequences', $Config->css_path);
...
<div class="container-wrapper">
	...
	<table class="concordtable" width="100%">
		...
		<?php echo print_freqlist_control_row($page_no, $next_page_exists, $resultCount, $pageCount); ?>
	</table>

	<table class="concordtable" width="100%">
		<tr>
			<th class="concordtable" width="5%">N°</th>
			<th class="concordtable" width="40%"><?php echo $att_desc[$att]; ?></th>
			<th class="concordtable">Fréquence</th>
		</tr>
...
function print_freqlist_control_row($page_no, $next_page_exists, $resultCount , $pageCount)
{
	...
	$nav_page_no['last']  = ($pageCount== $page_no ? 0 : $pageCount);
	/* all page numbers that should be dead links are now set to zero  */
	
	$string = '<tr>';

	$string .= '<td align="center" class="concordgrey"><b>Nombre des resultat: ';
	$string .= "$resultCount</b></td>";
		...
		'<form action="redirect.php" method="get">
			<td class="concordgrey">
				<select name="redirect">
					<option value="newFreqlist" selected="selected">Nouvelle Liste de Fréquences</option>
					<option value="downloadFreqList">Télécharger la liste</option>
...
function freqlist_write_download($att_desc, $description, &$result, $sum_tokens, $sum_types)
{
	...
	if (!$User->freqlist_altstyle)
	{
		...
		echo "N°\t$att_desc\tFréquence$eol$eol";
